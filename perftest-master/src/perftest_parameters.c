...
static void usage(const char *argv0, VerbType verb, TestType tst, int connection_type)
{
	...
	if (tst == BW) {
		...
		/*why version*/
		printf("  --iteration ");
		printf(" number of iteration, default 0 \n");
		printf("  --threshold");
		printf(" request quantity per iteration, default 0 \n");
		printf("  --gap ");
		printf(" gap between iteration, default 0\n");
		/*why version*/
	}
	...
	printf("  -D, --duration ");
	printf(" Run test for a customized period of microsecond.\n");
...
static void init_perftest_params(struct perftest_parameters *user_param)
{
	...
	/*why version*/
	// user_param->iteration=-1;
	// user_param->gap=0;
	// user_param->bytes_iter=0;
	/*why version*/
...
int parser(struct perftest_parameters *user_param,char *argv[], int argc)
{
	...
	/*why version*/
	static int iteration_flag = 0;
	static int gap_flag = 0;
	...
	while (1) {
		...
		static const struct option long_options[] = {
			...
			/*why version*/
			{ .name = "iteration", .has_arg = 1, .flag = &iteration_flag, .val = 1 },
			{ .name = "gap", .has_arg = 1, .flag = &gap_flag, .val = 1 },
			{ .name = "threshold", .has_arg = 1, .flag = &threshold_flag, .val = 1 },
			/*why version*/
			{0}
		};
		...
		switch (c) {
			...
			case 0: /* required for long options to work. */
				/*why version*/
				if (iteration_flag) {
					CHECK_VALUE(user_param->iteration, int, "Iteration count", not_int_ptr);
					iteration_flag = 0;
				}
				if (gap_flag) {
					CHECK_VALUE_IN_RANGE_UNS(user_param->gap, uint32_t, 0, UINT32_MAX, "Gap size", not_int_ptr);
					gap_flag = 0;
				}
				if (threshold_flag) {
					CHECK_VALUE_IN_RANGE_UNS(user_param->threshold, uint32_t, 0, UINT32_MAX, "Threshold value", not_int_ptr);
					threshold_flag = 0;
				}
				/*why version*/
				#ifdef HAVE_HNSDV
