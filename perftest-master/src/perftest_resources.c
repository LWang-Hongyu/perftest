...
int run_iter_bw_infinitely(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	...
	int 			return_value = 0;
	/*why version*/
	uint64_t 		*send_count_for_qp=calloc(num_of_qps, sizeof(uint64_t));
	int 			*iteration_for_qp=calloc(num_of_qps, sizeof(int));
	// iteration_for_qp[0]=2;
	// uint32_t threshold=3.7e6;
	// uint32_t gap=10;
	...
	/* main loop to run over all the qps and post each time n messages */
		for (index = 0 ; index < num_of_qps ; index++) {
			...
			while ((ctx->scnt[index] - ctx->ccnt[index] + user_param->post_list) <= user_param->tx_depth) {
				...
				/*why version*/
				send_count_for_qp[index] += user_param->post_list;
				/*why version*/
				totscnt += user_param->post_list;
		...
		if(send_count_for_qp[0]==threshold){
			if(iteration_for_qp[0]>0){
				gettimeofday(&end,NULL);
				sleep(gap);//sleep ${gap} seconds
				printf("\n[why debug] (sleeping) during time(s): %lu and iteration sequence: %d\n",(end.tv_sec-last.tv_sec),iteration_for_qp[0]);
				iteration_for_qp[0]--;
				send_count_for_qp[0]=0;
				gettimeofday(&last,NULL);
			}
			else {
				gettimeofday(&end,NULL);
...
void *handle_signal_print_thread(void* duration)
{
	...
	while(1){
		...
		struct timeval start,end;
		gettimeofday(&start,NULL);
		usleep(*duration_p*1000);
		/*****why version */
		//sleep(*duration_p);
		//printf("[why debug] now(ms): %lu \t",(start.tv_sec)*1000+(start.tv_usec)/1000);
		print_bw_infinite_mode();
		gettimeofday(&end,NULL);
		//printf("[why debug] during time(ms): %lu",(end.tv_sec-start.tv_sec)*1000+(end.tv_usec-start.tv_usec)/1000);
